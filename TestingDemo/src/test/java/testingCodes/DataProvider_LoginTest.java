...
public class DataProvider_LoginTest {
	...
	@Test(dataProvider = "loginData", dataProviderClass = DataProvider_Demo.class)
	public void loginTest(String username, String password, String expected) {
		...
		if (expected.equals("success")) {
			...
			Assert.assertTrue(currentUrl.contains("inventory"), "Login failed for valid user: " + username);
			...
			Assert.assertTrue(errorMsg.contains("locked out"), "Expected locked out error but got: " + errorMsg);
			...
			String errorMsg = driver.findElement(By.cssSelector("h3[data-test='error']")).getText();
			Assert.assertTrue(
					errorMsg.contains("Username and password do not match") || errorMsg.contains("do not match"),
...
/* if we dont use DataProvider then we would have used separate test methods
@Test
